...
if _swig_python_version_info >= (3, 0, 0):
    def swig_import_helper():
        import importlib
        #3.x library path
        file_path = libpath + '/lib/' + cpuType +'/3_x/_snowboydetect.so'
        mname = importlib.util.spec_from_file_location('_snowboydetect', file_path)
        try:
            return importlib.util.module_from_spec(mname)
        except ImportError:
            return importlib.util.module_from_spec(mname)
...
# 2.x
    ...
    def swig_import_helper():
        ...
        import imp
        #2.x library path
        file_path = [libpath + '/lib/' + cpuType +'/2_x']
        fp = None
        try:
            fp, pathname, description = imp.find_module('_snowboydetect', file_path)
        except ImportError:
